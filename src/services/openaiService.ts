...
export class OpenAINutritionService {
  ...
  async getNutritionData(foodName: string, weight: number = 100, unit: string = 'grams'): Promise<OpenAINutritionResponse> {
    const prompt = this.createContextAwarePrompt(foodName, weight, unit);
    
    console.log('OpenAI API Request:', { foodName, weight, unit, prompt });
  ...
  private getUnitAwareFallback(foodName: string, weight: number, unit: string): OpenAINutritionResponse {
